...
data City = City
    { cityCastle :: Castle
    , citySourceOfTruth :: SourceOfTruth
    , cityHouses :: [House]
    } deriving (Show)
...
data SourceOfTruth = Church | Library deriving (Show)

data House = OneOccupant | TwoOccupant | ThreeOccupant | FourOccupant  deriving (Show)

buildCastle :: String -> City -> City
buildCastle name city = city { cityCastle = c }
    where
        c = case cityCastle city of
            Castle _ -> Castle name
            WalledCastle _ -> WalledCastle name
            None -> None

buildHouse :: House -> City -> City
buildHouse house city = city { cityHouses = h }
    where
        h = house : cityHouses city

houseSize :: House -> Int
houseSize house = case house of
    OneOccupant -> 1
    TwoOccupant -> 2
    ThreeOccupant -> 3
    FourOccupant -> 4
...
population :: City -> Int
population city = sum (map houseSize (cityHouses city))

buildWalls :: City -> City
buildWalls city = if population city >= 10
    then
        case cityCastle city of
            Castle n -> city { cityCastle = WalledCastle n }
            _ -> city
    else city
{-
=🛡= Newtypes
...
data Dragon p = Dragon { power :: p }
data TreasureChest x = TreasureChest
    { treasureChestGold :: Int
    , treasureChestLoot :: x
    }
data Lair dragonPower tr = Lair
    { dragon :: Dragon dragonPower
    , treasure :: Maybe (TreasureChest tr)
    }
...
isWeekend w = (maxBound == w) || (minBound == w)
