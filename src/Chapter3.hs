...
data SpecialBuilding = Church | Library deriving (Show, Eq)

data Castle = Castle String (Maybe Wall) deriving (Show, Eq)

data Wall = Wall deriving (Show, Eq)

newtype House = House Int deriving (Show, Eq)
...
buildCastle city name = city {castle = Just (Castle name Nothing)}

buildHouse :: City -> Int -> City
buildHouse city ppl = city {houses = House ppl : houses city}

buildWalls :: City -> City
buildWalls city
  | isJust (castle city) && countPeople >= 10 = city {castle = fmap (\(Castle name _) -> Castle name (Just Wall)) (castle city)}
  | otherwise = city
  where
    countPeople = sum (map (\(House people) -> people) (houses city))
...
calculatePlayerDefense :: Armour -> Dexterity -> Defence
calculatePlayerDefense (Armour armor) (Dexterity dexterity) = Defence (armor * dexterity)
...
daysToParty day = case day of
  Monday -> "4 days to Friday"
  Tuesday -> "3 days to Friday"
  Wednesday -> "2 days to Friday"
  Thursday -> "1 day to Friday"
  Friday -> "It's Friday!"
  Saturday -> "6 days to Friday"
