...
fight (Fighter khealth kattack _) (Fighter mhealth mattack mgold)
  | (mhealth - kattack > 0) && (khealth - mattack > 0) = mgold
  | (mhealth - kattack > 0) && (khealth - mattack < 0) = -1
  | mhealth - kattack <= 0 = mgold
  | (mhealth - kattack > 0) && (khealth - mattack == 0) = 0
...
data Meal = Breakfast | Snacks | Lunch | Dinner
...
buildCastle newName city = case cityCastle city of
  CastleWithWalls _ -> city {cityCastle = CastleWithWalls newName}
  _ -> city {cityCastle = OnlyCastle newName}

buildHouse :: House -> City -> City
buildHouse house city =
  city {cityHouses = house : cityHouses city}

countNumberOfPeople :: House -> Int
countNumberOfPeople house = case house of
  One -> 1
  Two -> 2
  Three -> 3
  Four -> 4
...
countHouses houses = sum (map countNumberOfPeople houses)

buildWalls :: City -> City
buildWalls city = case cityCastle city of
  OnlyCastle castleName ->
    if countHouses (cityHouses city) >= 10
