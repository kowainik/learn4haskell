...
fight k m | knightAttack k < monsterHealth m = -1
...
turtleMonster = Monster {monsterName = "Turtle", monsterAttack = 1, monsterHealth = 30, monsterGold = 10}

firstRoundResult :: Int
firstRoundResult = fight arthur turtleMonster
...
newtype House = House {housePeople :: Int} deriving (Show)

newtype Castle = Castle {name :: String} deriving (Show)
...
data City where
  ...
  CastleCity ::
    ...
    City
  VillageCity ::
    {cityCulture :: Culture, cityHouses :: [House]} ->
    City
  deriving (Show)

peopleInCity :: [House] -> Int
peopleInCity houses = sum $ map housePeople houses

canBuildWalls :: [House] -> Bool
canBuildWalls houses = peopleInCity houses >= 10

buildCastle :: City -> String -> City
buildCastle city name = case city of
  VillageCity {cityCulture, cityHouses} -> CastleCity {cityCastle = Castle name, cityCulture = cityCulture, cityHouses = cityHouses}
  _ -> city {cityCastle = Castle name}

createHouse :: Int -> [House]
createHouse people
  | people > 0 && people < 5 = [House people]
  | otherwise = []

buildHouse :: City -> Int -> City
buildHouse city people = city {cityHouses = createHouse people ++ cityHouses city}

createCity :: String -> Culture -> [House] -> City
createCity name culture houses
  | canBuildWalls houses = WalledCity (Castle name) culture houses
  | otherwise = CastleCity (Castle name) culture houses

buildWalls :: City -> City
buildWalls city = case city of
  WalledCity {} -> city -- already has walls
  CastleCity {cityHouses, cityCulture, cityCastle} | canBuildWalls cityHouses -> WalledCity cityCastle cityCulture cityHouses
  _ -> city -- no castle or not enough people to build walls

auenland :: City
auenland = VillageCity Library $ createHouse 1

auenlandCannotBuildWalls :: City
auenlandCannotBuildWalls = buildWalls auenland

auenlandCannotBuildWallsEvenWithCastle :: City
auenlandCannotBuildWallsEvenWithCastle = buildWalls $ buildCastle auenlandCannotBuildWalls "Auenland Castle"

grownAuenland :: City
grownAuenland = buildHouse auenland 4

auenlandWithCastle :: City
auenlandWithCastle = buildCastle grownAuenland "Auenland Castle"

auenlandWithCastleAndWalls :: City
auenlandWithCastleAndWalls = buildWalls auenlandWithCastle
