...
-}
data Meal = Breakfast | SecondBreakfast | Brunch | Lunch | Dinner | Supper | Snack
{- |
=⚔️= Task 4
...
countPeople :: House -> Int  -- return number of people in a house
...
buildCastle city = case cityCastle city of
  ...
  _    -> city  -- will rebuild castle if it exists
...
buildWalls city = case cityCastle city of
  ...
  _ -> city -- can't build walls if there is no castle without walls in the city
