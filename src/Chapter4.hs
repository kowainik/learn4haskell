...
`x` is of type `Maybe a` while we require `Maybe b`
-}
...
merge Empty l2        = l2
merge l1 Empty        = l1
merge (Cons x xs) l2  = Cons x (merge xs l2)
...
instance Applicative List where
    ...
    (<*>) Empty _       = Empty
    (<*>) (Cons f fs) l = merge (fmap f l) (fs <*> l)
...
andM ma mb = ma >>= (\a -> fmap ((&&) a) mb)
-- andM ma mb = (fmap (\a -> (&&) a) ma) <*> mb
...
{-
Assuming reversing a tree means mirroring it along a vertical axis
-}
reverseTree :: Tree a -> Tree a
reverseTree EmptyTree     = EmptyTree
reverseTree (Node v l r)  = Node v (reverseTree l) (reverseTree r)

{-
Assuming pre-order traversal
-}
treeToList :: Tree a -> [a]
