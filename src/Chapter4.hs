...
joinList Empty l = l
joinList ls Empty = ls
joinList (Cons x xs) l = Cons x (joinList xs l)
...
instance Applicative List where
  ...
  Empty <*> _ = Empty
  _ <*> Empty = Empty
  (Cons x xs) <*> l = joinList (fmap x l) (xs <*> l)
...
flattenList (Cons x xs) = joinList x (flattenList xs)
...
treeToList (Node a l r) = Cons a (append (treeToList l) (treeToList r))
