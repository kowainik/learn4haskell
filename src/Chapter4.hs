...
instance Functor List where
  ...
  fmap _ Empty       = Empty
  fmap f (Cons a as) = Cons (f a) (f `fmap` as)
...
append Empty x = x
append x Empty = x
append (x `Cons` xs) a = x `Cons` (xs `append` a)
...
instance Applicative List where
  ...
  Empty <*> _ = Empty
  _ <*> Empty = Empty
  Cons f fs <*> l = (f `fmap` l) `append` (fs <*> l)
...
instance Monad List where
  ...
  l >>= f = concatList (f `fmap` l)

concatList :: List (List a) -> List a
concatList Empty = Empty
concatList (x `Cons` xs) = x `append` (concatList xs)
