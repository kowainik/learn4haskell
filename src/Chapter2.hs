...
subList start end l
    ...
    | otherwise =
        let 
            takeNumber = end - start + 1;
        in
        take takeNumber (drop start l)
...
takeEven l =  go [] l
    where
        go :: [int] -> [int] -> [int]
        go acc [] = acc
        go acc n = go (acc ++ (take 1 n)) (drop 2 n)
...
rewind l = go [] l
    where
        ...
        go acc n = 
            let
                readLimit = (length n) - 1
                lastElement = drop readLimit n
                newAcc = acc ++ lastElement
                restList = take readLimit n
            in
            go newAcc restList
