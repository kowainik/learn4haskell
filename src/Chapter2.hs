...
subList :: Int -> Int -> [a] -> [a]
subList _ _ [] = []
subList from to (x:xs)
    | from < 0 = []
    | from > 0 = subList (from - 1) (to - 1) xs
    | to < 0 = []
    | otherwise = x : subList 0 (to - 1) xs
-- subList from to lst@(x:xs)
--   | from < 0 = subList 0 to lst
...
firstHalf :: [a] -> [a]
firstHalf l = firstHalf' l (div (length l) 2)
    where
        firstHalf' _ 0 = []
        firstHalf' [] _ = []
        firstHalf' (x:xs) n = x : firstHalf' xs (n - 1)
