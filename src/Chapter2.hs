...
-}
subList :: Int -> Int -> [a] -> [a]
subList startIdx endIdx inList
    | startIdx < 0 || endIdx < 0 || endIdx < startIdx = []
    | otherwise = drop startIdx (take (endIdx + 1) inList)
...
-- smartReplicate l = map (\y -> replicate y y) l
-- not exactly sure how to use the hint
...
-}
--contains :: a -> [[a]] -> [[a]] Why cannot this be used? What can be used instead?
contains el l = filter (elem el) l
-- I know I could Eta-reduce but I won't correct it as I didn't know it when writing it
...
rotate idx l
    | null l || idx < 0 = []
    | otherwise = drop idx (take (length l + idx) (cycle l))
