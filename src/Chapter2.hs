...
-}
subList :: Int -> Int -> [a] -> [a]
subList lower upper list
  | lower < 0 || upper < 0 = []
  | upper < lower = []
  | upper == lower = (list !! lower) : []
  | otherwise = take numberOfElements (drop lower list)
  where numberOfElements = (+1) $ upper - lower
...
-}
firstHalf :: [a] -> [a]
firstHalf l = take numberOfElements l
  where numberOfElements = div (length l) 2
...
takeEven list = go True list
  where 
    go :: Bool -> [a] -> [a]
    go _ [] = []
    go isTake (x:xs) = if isTake then [x] ++ (go False xs) else (go True xs)
...
smartReplicate l = concat $ map replicate' l
  where replicate' x = replicate x x
...
contains n list = filter (\xs -> n `elem` xs) list
