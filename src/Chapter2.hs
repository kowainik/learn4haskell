...
subList :: Int -> Int -> [a] -> [a]
subList s e l =
  if s < 0 || e < 0 || null l then [] else drop s (take (e + 1) l)
...
takeEven = go 1
 where
  go :: Int -> [a] -> [a]
  go _   []       = []
  go acc (x : xs) = if mod acc 2 == 0 then rest else x : rest
    where rest = go (acc + 1) xs
...
contains :: (Eq a) => a -> [[a]] -> [[a]]
contains _ []       = []
contains c (x : xs) = if elem c x then x : rest else rest
...
-}
rotate :: Int -> [a] -> [a]
rotate x l = if x < 0 then [] else take (length l) (drop x (cycle l))
...
-- this version prepends to target list so it may be more efficient
rewind :: [a] -> [a]
rewind []  = []
rewind src = go [] src
 where
