...
subList :: Int -> Int -> [a] -> [a]
subList _ _ [] = []
subList b e xs | b > e || b < 0 || e < 0 = []
               | otherwise = drop b (take (e + 1) xs)
...
-}
takeEven :: [a] -> [a]
takeEven xs = takeEvenGo xs True []
  where
      takeEvenGo [] _ accs = accs
      takeEvenGo (x:xs) True accs = takeEvenGo xs False (accs ++ [x])
      takeEvenGo (_:xs) False accs = takeEvenGo xs True accs
