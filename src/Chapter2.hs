...
-}
subList :: Int -> Int -> [a] -> [a]
subList x y l = take (y - x + 1) (drop x l)
...
firstHalf :: [a] -> [a]
firstHalf l = take lLength l
  where
    lLength = length l `div` 2
...
contains d xs = filter (\a -> elem d a) xs
...
-}
rotate :: Int -> [Int] -> [Int]
rotate x xs = drop x $ take len (cycle xs)
  where
    len = length xs + x
