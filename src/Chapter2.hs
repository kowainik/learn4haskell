...
subList :: Int -> Int -> [a] -> [a]
subList firstPos lastPos list =
  if firstPos < 0 || lastPos < 0 then
    []
  else
    drop firstPos (take (lastPos + 1) list)
...
firstHalf l =
  let
    elementsInFirstHalf = div (length l) 2
  in
  take elementsInFirstHalf l
...
rotate n list =
  if n < 0 then
    ...
    take (length list) $ drop n $ List.cycle list
...
rewind list =
  case list of
    ...
    xs ->
      let
        remaining = take (length list - 1) xs
      in
      drop (length list - 1) xs ++ rewind remaining
