...
-}
subList :: Int -> Int -> [a] -> [a]
subList start stop xs
  | start < 0 || stop < 0 = [] -- return an empty list when given numbers are negative.
  | start > stop          = [] -- >>> subList 3 0 [True, False, False, True, False] = []
  | otherwise             = drop start $ take (stop + 1 ) xs
...
firstHalf :: [a] -> [a]
firstHalf l = fst $ splitAt (length l `div` 2) l
...
takeEven :: [a] -> [a]
takeEven [] = []
takeEven (x:_:xs) = x : takeEven xs -- base pattern
...
smartReplicate2 xs = concat $ map (\x -> replicate x x) xs
...
contains item xxs = filter (\xs -> elem item xs) xxs
...
rotate x xs
  | x < 0     = []
  | null xs   = []
  | otherwise = drop x $ take ((length xs) + x) $ cycle xs
...
rewind :: [a] -> [a]
rewind [] = []
rewind xs = lastElement : rewind (init xs)
    where
