...
-}
subList :: Int -> Int -> [a] -> [a]
subList b e l = if (b < 0 || e < 0) then [] else take (e-b+1) (drop b l)
...
log base 2, or the number of bits required to represent the number in binary
...
takeEven :: [a] -> [a]
takeEven [] = []
takeEven (zero:_:xs) = zero:takeEven xs
takeEven (zero:_) = [zero]
...
contains val list = filter (\l -> elem val l) list 
...
-}
rotate :: Int -> [Int] -> [Int]
rotate n l = if (n >= 0) then take (length l) (drop n (cycle l)) else []
