...
subList :: Int -> Int -> [a] -> [a]
subList num1 num2 arr 
  | num1 == num2 = take 1 (drop num1 arr)
  | num1 < num2 = take (num2 - num1 + 1) (drop num1 arr)
  | num1 < 0 || num2 < 0 = []
  | otherwise = []
...
firstHalf l 
  | null l == True = []
  | null l == False = take (div (length l) 2) l
...
contains x array = filter (\val -> elem x val ) array
...
rotate x arr 
  | x < 0 = []
  | x == 0 = arr
  | otherwise = take (length arr) (drop x (cycle arr))
