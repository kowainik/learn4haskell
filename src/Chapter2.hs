...
-}
subList :: Int -> Int -> [a] -> [a]
subList index1 index2 xs
  | index1 > index2 || index1 < 0 || index2 < 0  = []
  | otherwise = take (index2 - index1 + 1) (drop index1 xs)
...
smartReplicate l = concatMap (\a -> replicate a a) l
...
contains c listOfLists = filter (\x -> elem c x) listOfLists
...
-}
rotate :: Int -> [Int] -> [Int]
rotate n xs = subList n (length xs + n - 1) (cycle xs)
