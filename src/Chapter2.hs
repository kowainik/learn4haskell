...
functions defined in the special module called "Prelude". This module3
...
firstHalf aList =
  ...
  in take (div listLength 2) aList
...
smartReplicate aList = concat (map (\x -> replicate x x) aList)
...
contains n aList = filter (elem n) aList
...
rotate n aList
  | n < 0 || length aList == 0 = []
  | otherwise =
      take listLength (drop n (take repeatCycle (cycle aList)))
      where
        listLength = length aList
        repeatCycle = listLength * (n + 1)
