...
-}
takeEven [] = []
takeEven (x:y:z) = x : takeEven z
takeEven (x:z) = x : takeEven z
...
smartReplicate [] = []
smartReplicate (x:y) = (replicate x x) ++ smartReplicate y
...
-}
contains a [] = []
contains a (x:y) = if a `elem` x then x : (contains a y) else (contains a y)
...
rewind :: [a] -> [a]
rewind [] = []
rewind x = drop ((length x) - 1) x ++  rewind (init x)
