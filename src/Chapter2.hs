...
subList ll ul list
    | ll < 0 || ul < 0 = []
    | ll > ul = []
    | ll <= ul = drop ll (take (ul + 1) list)
    | otherwise = error "Unknown exception encountered!"
...
smartReplicate xs = concat [replicate x x | x <- xs]
