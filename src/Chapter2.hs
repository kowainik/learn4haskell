...
{- How can we do this with map + replicate? The closes I got was:
smartReplicate xs = concat (map (replicate x) xs)
where x = head xs
, but that just replicates each element by the first elements value.
Is there a way to get the current element value when using map? -}
...
-}
contains _ [] = []
contains x xs = filter (elem x) xs
