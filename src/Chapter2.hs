...
-}
subList :: Int -> Int -> [a] -> [a]
subList start stop list 
  | start < 0 = []
  | otherwise = take (stop - start + 1) (drop start list)
...
takeEven :: [a] -> [a]
takeEven [] = []
takeEven (x:xs) = takeEvenHelper True (x:xs)
  where
    takeEvenHelper :: Bool -> [a] -> [a]
    takeEvenHelper _ [] = []
    takeEvenHelper isEven (y:ys) = if isEven then y : takeEvenHelper False ys else takeEvenHelper True ys
...
rotate num xs
  | num < 0 = []
  | num == 0 = xs
  | otherwise = take (length xs) (drop num (cycle xs))
