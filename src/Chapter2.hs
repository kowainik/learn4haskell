...
import GHC.Base (VecElem(Int16ElemRep), Alternative (empty))

subList :: Int -> Int -> [a] -> [a]
subList x y zs 
  | (x>=0) && (y>=0) && (y>=x) = drop x (take (y+1) zs)
  | otherwise = []
...
-- 'go' is the recursion moved into a seperate function, it takes two numbers (end condition, input number n) of Int and returns the counts as Int (output number n) devided by 2 recursively. Maybe it could be a performance indicator like O-notation of its own function O(n).
...
-- return only even input
takeEven2 :: [Int] -> [Int]
takeEven2 [] = []
takeEven2 (x:xs)
  | even x = x : takeEven2 xs
  | odd x = takeEven2 xs
...
-- contains x = filter (\xs -> x `elem` xs)
-- contains x lol = filter (\xs -> x `elem` xs) lol
...
rotate :: Int -> [a] -> [a]
rotate x ys 
  | x >= 0 = drop x (take (x + length ys) $ cycle ys)
  | otherwise = []
...
rewind :: [a] -> [a]
rewind [] = []
rewind l = head rotateOneLess : rewind (tail rotateOneLess)
  where 
    rotateOneLess = rotate (length l - 1) l
-- rewind l = head (rotate (length l - 1) l) : rewind (tail (rotate (length l - 1) l))
-- rewind l = head (drop (length l - 1) l) : rewind (take (length l - 1) l)
