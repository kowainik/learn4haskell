...
subList c d ef
    | c < 0 = []
    | d < 0 = []
    | otherwise = (take ((d - c) + 1) (drop c ef))
...
contains p [] = []
contains p (x:xs)
  | p `elem` x = x : contains p xs
  | otherwise = contains p xs
...
rotate p xs
  | p <= 0 = []
  | otherwise = zipWith const (drop (abs (p)) (cycle xs)) xs
