...
subList x y l
    | x < 0 = []
    | y < 0 || y < x = []
    | otherwise = take (y - x + 1) . drop (x - 1) $ l
...
smartReplicate (x:xs) =
    replicate x x ++ smartReplicate xs

-- Got how to map each element with replicate but have no idea
-- how to concat all resulting lists togheter
