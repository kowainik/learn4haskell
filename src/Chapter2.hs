...
subList low high xs 
    ...
    | low <= high = take (high - low + 1) (drop low xs)
    | otherwise = []
...
takeEven xs = go 0 xs
  where
    go :: Int -> [a] -> [a]
    go _ [] = []
    go i (x:xs) = if (isEven i) then x : (go (i + 1) xs) else (go (i + 1) xs)
                  where
                    isEven :: Int -> Bool
                    isEven n = (mod n 2) == 0
...
contains n xs = filter (\x -> elem n x) xs
...
rotate n xs
  | n < 0  = []
  | otherwise = let l = length xs in (drop n (take (l+n) (cycle xs)))
