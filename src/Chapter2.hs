...
subList start end xs =
  if ((start >= 0) && (start <= end) && (end < length (xs))) then
    (tails (take (end + 1) xs) ) !! start
  else []
{- |
=⚔️= Task 4
...
firstHalf :: [a] -> [a]
firstHalf l = let halfN n = (div n 2) - 1 in
       subList 0 (halfN (length l)) l 
...
rotate :: Int -> [a] -> [a]
rotate n l 
    | n >= 0 && n < length l = subList n (length l - 1) l ++ subList 0 (n - 1) (l) 
    | otherwise              = [] 
