...
-}
firstHalf :: [a] -> [a]
firstHalf a = take (div (length a) 2) a
...
contains _ [] = []
contains n (x:xs) 
  | (isThreeIn n x) /= [] = (isThreeIn n x):(contains n xs)
  | otherwise = contains n xs
  where
    isThreeIn :: Int -> [Int] -> [Int]
    isThreeIn m y
      | (length (filter (==m) y))>0 = y
      | otherwise = []
...
rotate n x
  | n <= 0 = []
  | otherwise = take (length x) (drop n (cycle x))
...
rewind :: [a] -> [a]
rewind [] = []
rewind x = (drop ((length x)-1) x) ++ rewind (take ((length x)-1) x)
