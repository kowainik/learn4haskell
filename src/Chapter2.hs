...
-}
subList :: Int -> Int -> [a] -> [a]
subList a b li = if a < 0 || b <= 0 then [] else take (b-a+1) (cutBeforeRange a li)
    where 
      cutBeforeRange :: Int -> [a] -> [a]
      cutBeforeRange qty list = drop qty list
...
--Write a function that takes elements of a list only in even POSITIONS. (indexes?) (+_+)
takeEven :: [a] -> [a]
takeEven [] = [] 
takeEven [x] = [x]
...
-}
rotate :: Int -> [a] -> [a]
rotate num li = if num < 0 then [] 
  else getRot ( cutBeg (cycle li) )
    where 
      cutBeg :: [a] -> [a]
      cutBeg list = drop num list
      getRot :: [a] -> [a]
      getRot infList = take (length li) infList
{- |
=💣= Task 12*
...
rewind :: [a] -> [a]
rewind [] = []
rewind (first:others) = go others first []
    where
      go [] item revList  = revList ++ [item]
      go (x:xs) item revList  = go xs x revList ++ [item]
