...
subList start end xs
  ...
  | otherwise = take (end - start + 1) . drop start $ xs-- subList = error "subList: Not implemented!"
...
firstHalf :: [a] -> [a]
-- firstHalf x = fst (splitAt (div (length x + 1) 2) x)
...
listElementsLessThan :: (Ord a, Eq a) => a -> [a] -> [a]
