...
-}
subList :: Int -> Int -> [a] -> [a]
subList from to ls | from < 0 || to < 0 = []
                   | otherwise          = drop from . take (to + 1) $ ls
...
-}
firstHalf :: [a] -> [a]
firstHalf xs = let half = length xs `div` 2 in take half xs
...
-}
rotate :: Int -> [a] -> [a]
rotate i xs = if i < 0 then [] else take (length xs) . drop i $ cycle xs
