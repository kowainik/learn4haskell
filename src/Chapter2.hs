...
-}
subList :: Int -> Int -> [a] -> [a]
subList start end arr
	| ((end >= start) && (start >= 0)) = (drop start (take (end+1) arr))
	| otherwise = []
...
smartReplicate [] = []
smartReplicate l = (map (replicate (getElement l)) [(getElement l)])!!0 ++ smartReplicate (removeFirstElement l)
  	where
		getElement :: [Int] -> Int
		getElement [] = 0
		getElement list = (take 1 list)!!0
		removeFirstElement :: [Int] -> [Int]
		removeFirstElement [] = []
		removeFirstElement list = drop 1 list
...
contains i l
	| l == [] = []
	| elem i (l!!0) = l!!0 : contains i (drop 1 l)
	| otherwise = contains i (drop 1 l)
...
rotate i l
	| i < 0 = []
	| i == 0 = l
	| otherwise = drop i (take (i + length l) (cycle l))
...
rewind :: [Int] -> [Int]
rewind [] = []
rewind l = getNextElem i l
	where
		i = length l
		getNextElem :: Int -> [Int] -> [Int]
		getNextElem 0 _ = []
		getNextElem i list = list!!(i-1) : getNextElem (i-1) list
