...
subList p1 p2 xs 
   | p2 < p1 = []
   | otherwise = let rest = drop p1 xs 
                       in take (p2 - p1 + 1) rest
...
firstHalf :: [a] -> [a]
firstHalf xs = let l = length xs `div` 2
                  in take l xs
...
rotate n xs
   | n < 0 = []
   | otherwise = let rotation = take n xs 
                     in drop n xs ++ rotation
