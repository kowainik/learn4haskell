...
smartReplicate [] = []
smartReplicate xs = foldl (++) [] (map (\x -> replicate x x) xs)
--smartReplicate (x:xs) = replicate (abs x) x ++ smartReplicate xs
...
rotate n xs
  | n < 0 = []
  | n == 0 = xs
  | otherwise =  take (length xs) (drop (mod n (length xs)) (cycle xs))
