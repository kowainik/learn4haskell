...
-}
subList :: Int -> Int -> [a] -> [a]
subList l u ls = drop l (take (u+1) ls)
...
takeEven :: [Int] -> [Int]
takeEven [] = []
takeEven (x:xs) = isEven x ++ takeEven xs
    where isEven :: Int -> [Int]
          isEven n = if even n then [n] else []
...
smartReplicate ls = concat (map func ls)
    where func x = replicate x x
...
pairMul = (.) (map (uncurry (*)) .) zip
--pairMul = flip (.) zip (((.) . map . uncurry) (*))
