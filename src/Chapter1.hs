...
closestToZero x y = closest
    where
        absClosest :: Int
        absClosest = min (abs x) (abs y)
        closest    = if absClosest == abs x then x else y
...
sumLast2 n = firstDigit + secondDigit
  where
    absN = abs n -- handle negative numbers
    firstDigit  = mod absN  10
    secondDigit = mod (div (absN   - firstDigit) 10) 10
