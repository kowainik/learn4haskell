...
lastDigit :: Integer -> Integer --Integral a => a -> a didn't work for some reason
...
mid :: Ord a => a -> a -> a -> a
mid x y z 
    | min z (min x y) == x = if y > z then z else y
    | min z (min x y) == y = if x > z then z else x
    | otherwise = if y > x then x else y --z is min
