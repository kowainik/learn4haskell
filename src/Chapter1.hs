...
closestToZero x y = if (abs x) - (abs y) < 0 then x else y
...
firstBetween :: Int -> Int -> Int -> Bool
firstBetween x y z = (x <= y && x >= z) || (x <= z && x >= y)

mid :: Int -> Int -> Int -> Int
mid x y z
  | firstBetween x y z = x
  | firstBetween y x z = y
  | otherwise = z
...
sumLast2 :: Int -> Int
sumLast2 n = last1 (abs n) + last2 (abs n)
  where
    last1 :: Int -> Int
    last1 x = mod x 10
    last2 :: Int -> Int
    last2 x = div (mod x 100 - last1 x) 10
...
firstDigit :: Int -> Int
firstDigit n = 
  if x < 10 then x else firstDigit (div (x - mod x 10) 10)
  where x = abs n
