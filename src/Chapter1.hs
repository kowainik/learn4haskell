...
mid :: Int -> Int -> Int -> Int 
mid x y z 
    | ((x < y && y < z) || (z < y && y < x)) || (x == y) || (y == z) = y 
    | ((y < x && x < z) || (z < x && x < y)) || (x == z) = x 
    | otherwise = z  
...
isVowel c
    ...
    | otherwise = False
 -- simpler method would be   isVowel c = elem c ['a','e','i','o','u'] or isVowel c = (c == 'a' || c == 'e' ||c == 'i' ||c == 'o' ||c == 'u')
...
sumLast2 :: Int -> Int 
sumLast2 n = 
  let first_digit = mod n 10
      second_digit = mod (div n 10) 10
...
-}
firstDigit :: Int -> Int 
firstDigit n = if (div n 10 /= 0) then firstDigit (div n 10) else n
