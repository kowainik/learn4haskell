...
-}
isVowel :: Char -> Bool
isVowel c = let vowels = ['a', 'e', 'i', 'o', 'u', 'y'] in c `elem` vowels
...
sumLast2 :: Int -> Int
sumLast2 n = last2 $ abs n where
  last2 x = last1 x + last1 (x `quot` 10)
  last1 x = x `rem` 10
...
firstDigit :: Int -> Int
firstDigit n = firstDigit' (abs n) where
  firstDigit' x = if x < 10 then x else firstDigit (x `quot` 10)
