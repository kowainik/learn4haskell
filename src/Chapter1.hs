...
mid :: Int -> Int -> Int -> Int
mid x y z
  | (abs x < abs y) && (abs y < abs z) || (abs z < abs y) && (abs y < abs x) = y
  | (abs x < abs z) && (abs z < abs y) || (abs y < abs z) && (abs z < abs x) = z
  | otherwise = x
...
sumLast2 :: Int -> Int
sumLast2 n =
  let theLast = lastDigit n
      theLastButOne = lastDigit (div (abs n) 10)
   in theLast + theLastButOne
...
firstDigit :: Int -> Int
firstDigit n
  | abs n > 9 = firstDigit (removeLast n)
  | otherwise = abs n
  where
    removeLast :: Int -> Int
    removeLast n1 = div (abs n1) 10
