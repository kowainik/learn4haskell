...
mid :: Int -> Int -> Int -> Int
mid x y z
    | x > y = if y > z then y else if x > z then z else x
    | otherwise = if x > z then x else if y > z then z else y
...
-}
sumLast2 :: Int -> Int
sumLast2 n = lastOne n + secToLast n
  where
    lastOne :: Int -> Int
    lastOne n = mod (abs n) 10
    secToLast :: Int -> Int
    secToLast n = let nd = mod (div (abs n) 10) 10 in nd
...
firstDigit n 
    | div n 10 == 0 = abs n
    | otherwise = firstDigit (abs (div n 10))
