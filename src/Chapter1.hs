...
>>> :t True
True :: Bool       --Expectation: Bool
>>> :t 'a'
'a' :: Char        --Expectation: Char
>>> :t 42
42 :: Num p => p   --Expectation: Num

A pair of boolean and char:
>>> :t (True, 'x')
(True, 'x') :: (Bool, Char)    --Expectation: None

Boolean negation:
>>> :t not
not :: Bool -> Bool            --Expectation: Bool (guess)

Boolean 'and' operator:
>>> :t (&&)
(&&) :: Bool -> Bool -> Bool      --Expectation: Bool (guess)

Addition of two numbers:
>>> :t (+)
(+) :: Num a => a -> a -> a        --Expectation: None

Maximum of two values:
>>> :t max
max :: Ord a => a -> a -> a       --Expectation: None
...
>>> (3 + 5) < 10
True       --Expectation: True


>>> True && False
False       --Expectation: False
...
sumLast2 n =
  let lastDig = mod n 10
      secLastDig = mod (div n 10) 10
  in lastDig + secLastDig
