...
mid :: Int -> Int -> Int -> Int
mid x y z = minimum [max x y, max x z, max y z] -- max of every pair then the mid is forcibly the min of all the numbers, one-liner ;)
...
sumLast2 :: Int -> Int
sumLast2 n = second + first
  where
    lastTwoDigits = mod (abs n) 100
    (second, first) = divMod lastTwoDigits 10
...
firstDigit :: Int -> Int
firstDigit n = getRest (byTen)
  where
    byTen = divMod (abs n) 10
    getRest :: (Int, Int) -> Int
    getRest (0, lastDigit) = lastDigit
    getRest (rest, _) = getRest (divMod rest 10)
