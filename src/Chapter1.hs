...
-- 'a' :: Char
-- >>> :t 42
-- 42 :: Int | Apparently numeric constants are polymorphic so the correct answer is "42 :: Num a => a" but my first guess was correct when you use :t +d
...
--   Remember, that Haskell is an indentation- and layout-sensitive language. mid ::
mid :: (Ord a) => a -> a -> a -> a
mid x y z
  | x > y && y > z = y
  | y > z && z > x = z
